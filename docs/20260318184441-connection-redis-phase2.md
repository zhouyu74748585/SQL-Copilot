...
## ????
- `apps/desktop/src/modules/studio/components/StudioShell.vue`
  - ? Redis ????????? `redis-browser-table`??????????? Redis ??????
- `apps/desktop/src/modules/studio/styles/shell.css`
  - ?? Redis ???? Ant Design ???????????
  - ? Redis PATH ??? `cursor: pointer`???????????/?????????

## ????
- ?????????`npm run -w @sqlcopilot/desktop type-check`
- ???????`npm run -w @sqlcopilot/desktop build`
- ?? clean ????????`mvn -f apps/server/pom.xml clean spring-boot:run -Dspring-boot.run.arguments=--server.port=18097`
- ?? preview ????????`npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4185 --strictPort`

## ??
- ???? API ??????? `http://localhost:18080`??? clean ????? `18080` ????????????????? `18097`?
- ?????????????????? `npm install` ?????
