...
- ???
  - `release/desktop-validation/mac-arm64/SQL-Copilot-0.1.0-mac-arm64.zip`
  - `release/desktop-validation/mac-x64/SQL-Copilot-0.1.0-mac-x64.zip`
...
- ????? `http://127.0.0.1:18080/api/health` ?? `200` ??????? `release/desktop-runtime-validation/win-x64/win-unpacked/SQL Copilot.exe`???????? backend ??? `18081`?
- ???????????? backend?
...
- mac ???????
  ...
  - ???`release/desktop-mac-ci-validation/mac-arm64/SQL-Copilot-0.1.0-mac-arm64.zip`
- clean ?????
...
## ????
- ?? workflow ?????? `release/desktop/${target}/**`??????????????????
- ????????????? `exe`?`zip`?`tar.gz`??????????????????
  - Windows?`win-unpacked/`?`builder-debug.yml`?`.blockmap`
  - macOS???? `.app` ??
  - Linux?????
- ?? GitHub artifact ???????????????????????????????/????????? GitHub ????????
- ??????????????
  - `release/desktop/mac-arm64` ???? `2081646526` ?????? `SQL-Copilot-0.1.0-mac-arm64.zip` ? `833721825` ???
  - `release/desktop/win-x64` ???? `2292471401` ?????? `SQL-Copilot-0.1.0-win-x64.exe` ? `793658767` ???

## ????
- ?? `.github/workflows/package-variants.yml`??????????????
  - Windows?`release/desktop/win-x64/*.exe` ? `*.blockmap`
  - macOS?`release/desktop/mac-arm64/*.zip`?`release/desktop/mac-x64/*.zip`
  - Linux?`release/desktop/linux-x64/*.tar.gz`
- ?????????????? `win-unpacked` / `.app` ??????????? GitHub artifact ?????

## ????
- workflow ???????
- clean ?????
  - ???`mvn.cmd -f apps/server/pom.xml clean spring-boot:run -Dspring-boot.run.arguments=--server.port=18086`?`GET /api/health` ?? `ok`?
  - ???`npm run -w @sqlcopilot/desktop build -- --emptyOutDir` ??`npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4193 --strictPort` ?? `HTTP 200`?
...
### ????
- ?? `.github/workflows/package-variants.yml`?
  - ? `workflow_dispatch` ?? `tag_name`?`release_name`?`draft`?`prerelease` ???
  - ?? `prepare-release` ????????????????????????? GitHub API ???????? Release ? tag?
  - ??????????????????? `publish-release` ???????????????/?? GitHub Release?
  - workflow ??? `contents: read` ??? `contents: write`??????????
- ?? `scripts/generate-release-notes.mjs`?
  - ?? git log ???? Release ??? Commit ?? merge ???
  - ??????? / ??????? / ???????? / ????? / ????? / AI ??????????????
  - ?????????????????????????????????? compare ???
- ?????????? tag ?????? `v<desktop-version>-build.<run_number>`???????????????????
...
### ???
- ????? workflow ????????????? GitHub ?????????? Action?????????? dispatch ?????? tag ?? Release ????????????


### 2026-03-26 13:22:17

## ????
- ?? Windows ??? `npm run package:app` ???? `vue-tsc` ??????
- ?????????????????????????? `apps/server/target` ?? Maven clean ???

## ????
- ???????
  - ???????? `npm install`??? workspace ???????????? `vue-tsc`?`vite`?`electron-builder` ??????
- ???????
  - ?? `scripts/package-variants.mjs`?
  - ??????????? `buildBackend()` ? `mvn clean package -DskipTests` ??? `mvn package -DskipTests`??? Windows ????? `sql-copilot-server-0.1.0.jar` ???? `target` ??????????
  - ? `locatePackagedJar` ???????????????????????
...
- ??????????
  ...
  - `npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4174 --strictPort` ?????HTTP 200?
- ?????
  - `mvn clean package` ? `apps/server` ????
  - ?????? `18081` ?? `target/sql-copilot-server-0.1.0.jar` ???`GET /actuator/health` ?? 200??????? `Started SqlCopilotApplication`?
  - ???????? `18080`??????????? `18081` ??????????
- ???????
  - `npm run package:app` ?? 2026-03-26 13:15:17?Asia/Shanghai??????
  - ??????
    - `release/desktop/win-x64/SQL-Copliot-1.2.0-win-x64.exe`
    - `release/desktop/mac-arm64/SQL-Copliot-1.2.0-mac-arm64.zip`
    - `release/desktop/mac-x64/SQL Copliot-darwin-x64`?????????
    - `release/desktop/linux-x64/SQL Copliot-linux-x64`?????????

## ??
- ?? `package-lock.json` ??????????????????????? `scripts/package-variants.mjs`?
...
## ????
- ?? Windows ?????? mac ????? macOS ?????????
- ?? GitHub ? mac runner ????????????? mac ??? mac ?????

## ????
- ?? `scripts/package-variants.mjs`?
  - ?? mac ??? mac ?????? `.zip` ?????
  - ???? mac ??????? macOS ?????? `.zip`????? `.tar.gz`??? Windows ?????? `.app` ?????
- ?? `scripts/create-macos-archive.py`?
  - ?? Python `tarfile` ?? mac ???
  - ??????????? framework ?? symlink?
  - ? symlink ?????? POSIX `/` ??????? Windows ??????
  - ? `.app/Contents/MacOS/*`?framework Mach-O ????`backend/run.sh`?mac ?? `qdrant` ??????? `0755`?
  - ?????? `0644`????? `0755`?

## ????
- Windows ??????????
  - `node scripts/package-variants.mjs --targets=mac-arm64` ???????????
  - ??? `release/desktop-mac-perm-validation-2/mac-arm64/SQL-Copliot-1.2.0-mac-arm64.tar.gz`?
- ???????
  - `SQL Copliot.app/Contents/MacOS/SQL Copliot` ? `-rwxr-xr-x`?
  - `SQL Copliot Helper.app/Contents/MacOS/SQL Copliot Helper` ? `-rwxr-xr-x`?
  - `Electron Framework.framework/Versions/A/Electron Framework` ? `-rwxr-xr-x`?
  - `Contents/Resources/backend/run.sh` ? `-rwxr-xr-x`?
  - `Squirrel.framework/Resources -> Versions/Current/Resources` ? symlink ?????????
- ?????
  - ???`mvn clean package` ??????? `java -jar target/sql-copilot-server-0.1.0.jar --server.port=18082` ?????`GET /actuator/health` ?? 200?
  - ???`npm run -w @sqlcopilot/desktop build` ???`npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4175 --strictPort` ?? HTTP 200?

## ??
- ?? GitHub Actions ? `mac-arm64` ? `mac-x64` ?????? macOS runner ??????????? `.zip`?????? Windows ????????
...
## ????
- ?????????????????? qdrant ?????????????????
- ???????????????????? `Resources/qdrant/<platform-key>/...` ?? qdrant?

## ????
- ?? `apps/desktop/scripts/prepare-qdrant-package.mjs`?
  - ?????????????? `apps/desktop/resources/qdrant-package/qdrant/<platform-key>`?
  - ???????????????? qdrant ????
- ?? `apps/desktop/package.json`?
  - `build.extraResources[0].from` ? `resources/qdrant` ?? `resources/qdrant-package/qdrant`?
  - ?? `electron-builder` ????????????????????
- ?? `scripts/package-variants.mjs`?
  - ??? target ????? `preparePackagedQdrant(target)` ???
  - `electron-packager` ?????????? qdrant??? mac/linux ????????????
  - `cleanupStageDir()` ??? `resources/qdrant-package` ??????????????????
- ?? `.gitignore`?
  - ?? `apps/desktop/resources/qdrant-package`?

## ????
- mac x64 ?????
  - ???`release/desktop/mac-x64/SQL-Copliot-1.2.0-mac-x64.tar.gz` = 905.55 MB
  - ???`release/desktop-size-validation-2/mac-x64/SQL-Copliot-1.2.0-mac-x64.tar.gz` = 833.10 MB
  - ???72.44 MB
- ??????
  - ?? mac x64 ???????
    - `SQL Copliot.app/Contents/Resources/qdrant/darwin-x64/qdrant`
  - ???? `darwin-arm64`?`linux-x64`?`win32-x64` ????? qdrant?
- Windows ???????
  - `release/desktop-win-size-validation/win-x64/win-unpacked/resources/qdrant/win32-x64/qdrant.exe` ???? `electron-builder` ?????? Windows ????????? qdrant ???
- ?????
  - ???`mvn clean package` ????? `java -jar target/sql-copilot-server-0.1.0.jar --server.port=18084` ?????`GET /actuator/health` ?? 200?
  - ???`npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4176 --strictPort` ?? HTTP 200?

## ??
- ???????????????????? `backend/sql-copilot-server-0.1.0.jar`??? JRE ? Electron Framework?qdrant ??????????
...
## ????
- ????????????????????????????
- ??????????????? backend jar??? Windows ????????? GPU ? backend ?????? mac ???? CPU ? backend ?????? Windows ???

## ????
- ?? `scripts/package-variants.mjs`?
  - ? backend ????????????????????? target ??????????
  - ?? `resolveBackendOnnxRuntimeArtifactId(target)`?
    - `mac-*` ???? `onnxruntime`
    - `win-*` / `linux-*` ???? `onnxruntime_gpu`
  - ??? Maven ??????
    - `-Donnxruntime.artifactId=onnxruntime`
    - ? `-Donnxruntime.artifactId=onnxruntime_gpu`
- ????? `apps/server/target` ??????????????? backend ?????????
  - ?? target ??? `apps/server` ? `release/.backend-build-stage/<target>`
  - ???????? `mvn clean package -DskipTests ...`
  - ????? `target/` ?? jar ????? jlink / ??????
- ???????????
  - ?????????? OS ??
  - ? target ?? clean ????????? `apps/server/target/*.jar` ?????

## ????
- ?????????
  - ???`node scripts/package-variants.mjs --targets=mac-x64,win-x64`
  - ?????
    - `backend:mac-x64 package (onnxruntime)`
    - `backend:win-x64 package (onnxruntime_gpu)`
- ???????
  - `release/desktop-backend-target-validation-2/mac-x64/.../backend/sql-copilot-server-0.1.0.jar`
    - ???199.47 MB
    - ???`BOOT-INF/lib/onnxruntime-1.19.2.jar`
  - `release/desktop-backend-target-validation-2/win-x64/.../backend/sql-copilot-server-0.1.0.jar`
    - ???533.77 MB
    - ???`BOOT-INF/lib/onnxruntime_gpu-1.19.2.jar`
- ?????
  - ???`mvn clean package` ????? `java -jar target/sql-copilot-server-0.1.0.jar --server.port=18085` ?????`GET /actuator/health` ?? 200?
  - ???`npm run -w @sqlcopilot/desktop preview -- --host 127.0.0.1 --port 4177 --strictPort` ?? HTTP 200?

## ??
- ?????????????????????? ONNX Runtime????? `apps/server` ?????????
- ????????????????????????????? `scripts/package-variants.mjs`?
...
## ????
- ?? Windows ?????? mac ????? xattr ????? `Backend startup timeout` ????
- ?????? backend JRE ?????????????????

## ????
- Electron ???????????? `.app` ??????????
- backend ??????? `resources/backend/jre` ?? Java ???????????
- ???????? mac ????
  - `backend/jre/bin/java` ? `rwxr-xr-x`
  - ? `backend/jre/lib/*.dylib` ? Mach-O ???? `rw-r--r--`
- ????? `libjli.dylib`?`libjvm.dylib` ? Java ????????? Windows ??????????????????? backend ?????????? `Backend startup timeout`?

## ????
- ?? `scripts/create-macos-archive.py`?
  - `should_be_executable()` ?????
  - ???? `backend/run.sh`?`backend/jre/bin/*`???? `Contents/MacOS/*`?framework Mach-O ? qdrant ?? `0755`?
  - ??????`/Contents/Resources/backend/jre/` ??? Mach-O ??????? `0755`?
  - ?? `libjli.dylib`?`libjvm.dylib` ? JRE ?????????????????

## ????
- ???????
  - `node scripts/package-variants.mjs --targets=mac-arm64`
  - ???`release/desktop-mac-backend-fix-validation/mac-arm64/SQL-Copliot-1.2.0-mac-arm64.tar.gz`
- ???????
  - `SQL Copliot.app/Contents/Resources/backend/jre/bin/java` = `-rwxr-xr-x`
  - `SQL Copliot.app/Contents/Resources/backend/jre/lib/libjli.dylib` = `-rwxr-xr-x`
  - `SQL Copliot.app/Contents/Resources/backend/jre/lib/server/libjvm.dylib` = `-rwxr-xr-x`
- ?????
  - ???? `mac-arm64` ???? 407.35 MB?????? mac ?? CPU ? ONNX backend ???? qdrant?

## ??
- ???????? macOS????????????? `.app` ?????????????????????????????
- ??? mac ?????????? `.app` ??????`xattr -cr <SQL Copliot.app>`??????? backend ?????????
